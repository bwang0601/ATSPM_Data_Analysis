...
There are total ten signals included on the 800N corrdior
```{r measures Dataset}
...
PCD <-read_xls("data/800N/800N_PCD.xls") %>% select(-Id) %>%
  mutate(
    BinStartTime = as_datetime(BinStartTime),
...
SF <-read_xls("data/800N/800N_SplitFail.xls") %>% select(-Id) %>%
  mutate(
    BinStartTime = as_datetime(BinStartTime),
...
PPT <-read_xls("data/800N/800N_PPT.xls") %>% select(-Id) %>%
  mutate(
    BinStartTime = as_datetime(BinStartTime),
    SignalId = as.numeric(SignalId)
  )
...
df <- left_join(
  ...
  by = c("BinStartTime","ApproachId")) %>% unique()
...
df3 <- left_join(
  df2, PPT, 
  by = c("BinStartTime", "SignalID" = "SignalId")) %>% unique()
...
df7 <- df6 %>% select(-year) %>% select(-Volume) %>%
  mutate(
